...
const Login = () => {
  ...
  useEffect(() => {
    if (user) {
      // Vérifier si le playerIdFromDevice est disponible
      const playerIdFromDevice = window.__oneSignalPlayerId || null;
      const playerIdFromRedux = user?.onesignal_player_id;
      
      console.log("Login - Vérification Player ID:", {
        playerIdFromRedux,
        playerIdFromDevice
      });
      
      // Si le playerIdFromRedux est null ou différent du playerIdFromDevice
      if (!playerIdFromRedux || !playerIdFromDevice || playerIdFromRedux !== playerIdFromDevice) {
        console.log("Redirection vers /Notif pour autoriser les notifications");
